...
############################## 국내주식    ###########################
    ...
    def kor_stk(self):
        ...
        ace = """
         SELECT ETF_NM NM fROM quant1.dbo.FN_ETFDATA WHERE TR_YMD=(SELECT MAX(TR_YMD) FROM quant1.dbo.FN_ETFINFO WHERE TR_YMD<=GETDATE()) AND ETF_CD IN (SELECT STk_CD FROM quant1.dbo.ES_fUND_MAP) order by ETF_NM ASC
         """
        ace=DB.read(ace,conn) 
        
        ex_ace = """
         SELECT ETF_NM NM fROM quant1.dbo.FN_ETFDATA WHERE TR_YMD=(SELECT MAX(TR_YMD) FROM quant1.dbo.FN_ETFINFO WHERE TR_YMD<=GETDATE()) AND ETF_CD not IN (SELECT ISNULL(STk_CD,'A452250') FROM quant1.dbo.ES_fUND_MAP) order by ETF_NM ASC
         """
        ...
        if page=='ACE':
            ...
            option1 = ace['NM'].tolist()
            overview=fund_asset[~fund_asset['FUND_NM'].isin(option1)]
            overview=overview[overview['FUND_FNM'].str.startswith('한국투자')]
            
        elif page=='타사':    
            option1 = st.selectbox("ETF명", ex_ace['NM'],key=12)
       ...
       # overview=fund_asset[fund_asset['FUND_NM']==option1]
        ...
        gb['pinnedBottomRowData'] = [{'FUND_FNM':'전체(합계)','평가액':overivew_f['평가액'].sum()}]
        ...
        gb['columnDefs'] = [ { 'headerClass': 'blank', 'children': [ { 'field': 'FUND_FNM','width':500},{ 'field': 'ETF_NM'}  ] }, 
