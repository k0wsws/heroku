...
############################## 국내주식    ###########################
    ...
    def kor_stk(self):
        ...
        gb['pinnedBottomRowData'] = [{'CO_NM':'합계','펀드수':comp_fin['펀드수'].sum(),'Δ펀드수':comp_fin['Δ펀드수'].sum(),'순자산':comp_fin['순자산'].sum(),'M/S(%)':comp_fin['M/S'].sum(),'Δ순자산(D1)':comp_fin['Δ순자산(D1)'].sum(),'Δ순자산':comp_fin['Δ순자산'].sum(),'Δ설정액(D1)':comp_fin['Δ설정액(D1)'].sum(),'Δ설정액':comp_fin['Δ설정액'].sum(),'개인':comp_fin['개인'].sum(),'은행':comp_fin['은행'].sum(),'보험':comp_fin['보험'].sum(),'외국인':comp_fin['외국인'].sum(),'투신':comp_fin['투신'].sum(),'기타법인':comp_fin['기타법인'].sum(),'사모펀드':comp_fin['사모펀드'].sum()}]
